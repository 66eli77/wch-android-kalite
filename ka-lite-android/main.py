...
class ServerThread(threading.Thread, Server):
    ...
    def schedule_reload_content(self):
        JavaHandler.displayInLogCat("cdcdcd in")
        if self.server_is_running:
            time.sleep(2)
            JavaHandler.displayInLogCat("cdcdcd if 1")
            # from android import AndroidService
            #JavaHandler.displayInLogCat("cdcdcd if 2")
            # AndroidService().stop()
            self.stop_server()
            JavaHandler.displayInLogCat("cdcdcd if 3")
        if JavaHandler.movingFile():
            JavaHandler.displayInLogCat("cdcdcd if 4")
            self.start_server('threads=18')
            JavaHandler.displayInLogCat("cdcdcd if 5")
            return 'Loading finished'
        else:
            JavaHandler.displayInLogCat("cdcdcd if 6")
            self.web_view.show_toast('Content  folder  not  found!  Exiting...')
            time.sleep(4)
            JavaHandler.killApp()
